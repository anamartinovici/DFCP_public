...
```{r load-data}
library("tidyverse")
load("recent_search/processed_data.RData")
```
...
df_tweets %>% filter(geo_place_id != "no_geo_info") %>% select(text, geo_place_id)

# add the name of that location
df_tweets <- df_tweets %>% left_join(df_places, by = c("geo_place_id" = "place_id"))
df_tweets %>% filter(geo_place_id != "no_geo_info") %>% select(text, geo_place_id, 
                                                               place_full_name,
                                                               place_name)

df_ref_tweets <- df_ref_tweets %>% left_join(df_places, by = c("ref_geo_place_id" = "place_id"))
df_ref_tweets %>% 
    filter(ref_geo_place_id != "no_geo_info") %>% 
    select(ref_text, ref_geo_place_id, 
           place_full_name,
           place_name)
...
Adding the length of the tweet text and checking if there are differences between different types of tweets.

```{r length-of-text-1}
df_tweets <- df_tweets %>% mutate(text_length = str_length(text))
df_tweets %>% group_by(tweet_type) %>% summarise(max_length = max(text_length))

df_ref_tweets <- df_ref_tweets %>% mutate(ref_text_length = str_length(ref_text))
df_tweets <- df_tweets %>% 
    left_join(df_ref_tweets %>% select(ref_tweet_id, ref_text, ref_text_length),
              by = "ref_tweet_id")
df_tweets %>% group_by(tweet_type) %>% summarise(max_length = max(text_length),
...
Let **Tweet 1** be the id of an "original tweet". Then, if this original tweet gets retweeted, the let the id of the RT be **Tweet 2**. The retweet count associated with **Tweet 2** is the same as that associated with **Tweet 1**. The figure below show this for our sample:
